            ...
            {experiences.map((exp) => (
              <div key={exp.id}>
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs font-semibold text-slate-700 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded shrink-0">
                    {exp.startDate} — {exp.isCurrent ? 'Sekarang' : exp.endDate}
                  </span>
                </div>
                {exp.description && (
                  <p className="mt-2 text-slate-800 text-justify whitespace-pre-line leading-relaxed text-xs">
                    {exp.description}
                  </p>
                )}
              </div>
